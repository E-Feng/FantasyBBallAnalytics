...
def transform_draft_recap(draft: pd.DataFrame, players: pd.DataFrame, settings: pd.DataFrame):
  ...
  players['ratingSeason'] = players['statRatingsSeason'].apply(calculate_ratings_no_ejections)

  players['rankingSeason'] = players['ratingSeason'].rank(method='min', na_option='keep', ascending=False)
